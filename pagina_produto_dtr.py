...
class PaginaProdutoDtr(PaginaProduto):
    ...
    def disponibilidade(self) -> bool:
        try:
            text = self.pagina_produto.find("div", {"class": "btn_aviseme interna"}).get_text()
    ...
    def existe_mais_de_um_sku(self) -> bool:
        ...
        try:
            text_list = self.pagina_produto.find("div", class_="produtoSelect").text.split(" ")
    ...
    def identificar_grade_selecionada(self) -> str:
        try:
            grade_ja_selecionada = self.pagina_produto.find('span', 'variacao-volt active').text.split()[0].\
                upper().replace('V', '')
        except AttributeError:
    ...
    def coletar_preco(self) -> tuple:
        try:
            preco_vista = self.pagina_produto.find("span", {"data-locale": "pt_BR"}).text.replace('.', ',')
        except AttributeError:
            preco_vista = ''

        try:
            preco_prazo_phrase = self.pagina_produto.find("div", class_="ou-sem-juros").text.replace('&nbsp', ' ')\
                .split()
            preco_prazo = (preco_vista if preco_prazo_phrase[2].isalpha() else preco_prazo_phrase[2])
        except AttributeError:
            preco_prazo = ''
