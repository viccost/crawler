...
class PaginaProdutoB2U(PaginaProduto):
    ...
    def disponibilidade(self) -> bool:
        obj = self.pagina_produto.find("button", {"class": "styles__Button-sc-13a2o83-3 llNZvE"})
        if obj:
            disponivel = False
        else:
            disponivel = True
    ...
    def existe_mais_de_um_sku(self) -> bool:
        existe_grade = False
        try:
            text = self.pagina_produto.find("div", {"class": "old__Type-sc-uaqko9-6 miAcb"}).text
    ...
    def identificar_grade_selecionada(self) -> str:
        try:
            ...
            try:
                grade_ja_selecionada = self.pagina_produto.find("strong", class_="old__Bold-sc-uaqko9-3 ldVOLn").text
            except AttributeError:
    ...
    def coletar_preco(self) -> tuple:
        try:
            preco_vista = self.pagina_produto.find("div",
                                                   {"class": "styles__PriceText-sc-x06r9i-0 dUTOlD priceSales"}).text
        except AttributeError:
            preco_vista = ""
        preco_prazo = ""
